...
ice_area_month <- aggregate(area_winter, area_ice ~ year_winter*month, FUN = "mean")
...
p.all_ice <- ggplot(all_ice, aes(x = area_ice/10000, y = ws_count_sic_ge95))+
  geom_line()
p.all_ice
...
m_ice <- lm((all_ice$area_ice/1000) ~ all_ice$ws_count_sic_ge95)
summary(m_ice)
modelplot(m_ice)

p.ice_days <- ggplot(all_ice)+
              geom_line(aes(x = all_ice$year_winter, y = all_ice$ws_count_sic_ge95))+
              geom_line(aes(x = all_ice$year_winter, y = all_ice$area_ice/10000))
p.ice_days

# Bering Sea sea ice EXTENT ####

# load spreadsheet for sea ice AREA
extent_all <- readxl::read_excel("data/daily_si_extent_nsidc/daily_si_ext_nsidc_wide_20241010.xlsx")
str(extent_all)

# create julian date as sequence of 1 to 366 days
extent_all$julian <- seq(1,366,1)

# replace NAs with -9
#area_all[, 3:48][is.na(area_all[,3:48])] <- -9

# transpose from wide to long with r base function reshape with reshape2 function melt
extent_all_long <-melt(extent_all, id.vars = c("month","day", "julian"), variable.name = "year", value.name = "extent_ice")
str(extent_all_long)
extent_all_long$year <- as.numeric(as.character(extent_all_long$year))

# subset for winter-spring months (01 Nov - 30 Apr)
extent_winter <- subset(extent_all_long, month > 10 | month < 5)

# create winter-year variable to identify winter by its starting year
extent_winter$year_winter <- ifelse(extent_winter$month > 10, extent_winter$year, extent_winter$year+1)

# summarize ice area mean by year
ice_extent_year <- aggregate(extent_winter, extent_ice ~ year_winter, FUN = "mean")

p.ice_extent_year<-ggplot(ice_extent_year, aes(x=year_winter, y=extent_ice))+
  geom_line()
p.ice_extent_year
...
# merge ice days with ice area
all_ice <- merge(ice_days, ice_extent_year, all.x = TRUE, by = "year_winter")

p.all_ice <- ggplot(all_ice, aes(x = extent_ice/10000, y = ws_count_sic_ge95))+
  geom_line()
p.all_ice

ice_cor <- cor(all_ice$extent_ice, all_ice$ws_count_sic_ge95)
ice_cor

m_ice <- lm((all_ice$extent_ice/1000) ~ all_ice$ws_count_sic_ge95)
summary(m_ice)
modelplot(m_ice)

p.ice_days <- ggplot(all_ice)+
  geom_line(aes(x = all_ice$year_winter, y = all_ice$ws_count_sic_ge95))+
  geom_line(aes(x = all_ice$year_winter, y = all_ice$extent_ice/10000))
p.ice_days
